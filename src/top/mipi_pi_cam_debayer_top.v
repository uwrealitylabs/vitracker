...
module mipi_pi_cam_debayer_top
#(
)
(
...
//-----------------------------------------------------------//
parameter MIPI_H_PIXEL 	= H_ActivePix >> 2;
//-----------------------------------------------------------//
...
reg			r_out_hsync_1P;
reg			r_out_vsync_1P;
reg	[39:0]	r_out_data_1P;
reg			r_fb_reset;

wire[19:0]	w_resync_data_3P;
wire[19:0]	w_resync_data_0;
wire[19:0]	w_resync_data_1;
wire[39:0]	w_resync_data;
...
begin
	...
	begin
		...
		r_x_mipi_1P	<= (r_x_mipi + (r_x_pointer >> 1));
		r_y_mipi_1P	<= (r_y_mipi + r_y_pointer);
...
wire	[10:0]	debug_r_wr_y;   
wire	[10:0]	debug_axi_y_wr;   
wire			debug_r_rd_en;
wire	[32:0]	debug_w_wr_data_00;
...
begin
	if (~resetn)
		flash_cnt <= 5'b0;
	else if (mipi_inst1_ERROR[9])	// bit9 will be "1"	if there is no error
	begin
		vsync_LED	<= mipi_inst1_VSYNC[0];
		if (!vsync_LED && mipi_inst1_VSYNC[0])
			flash_cnt <= flash_cnt + 1'b1;
		else if (flash_cnt == 5'b11111)
			flash_cnt <= 1;
	end
end
